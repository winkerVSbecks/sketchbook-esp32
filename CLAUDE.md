...
| `a18` | Waveshare ESP32-S3-Touch-AMOLED-1.8 (V2) | 368x448 AMOLED over QSPI (ESP32-S3R8, 8MB octal PSRAM, CST820 touch) |
...
| `src/the_core.cpp` | `core_a18*` | still between gestures; six nested isometric cubes — tap replays the stagger-bounce pop, holding shrinks the stack diagonally shut, release springs it back open like jelly. Deterministic — no randomness, RESET changes nothing (touch-first) |
| `src/napoleon.cpp` | `nap_a18*` | still between gestures; Napoleon's theorem as a fidget — drag a purple vertex handle and the outer equilateral triangles plus their centroid triangle (always equilateral: the theorem) follow live; a quick tap on empty glass resets the layout. Deterministic — no randomness (touch-first). SDL: mouse = finger |
| `src/switcher_kids.cpp` | `kids_t2*`, `kids_a18*` | paint + zhi + the core + napoleon in one binary; BOOT swaps, the glass belongs to the active sketch — this switcher must not read taps (see its header). On the a18 (no RESET) the fresh-composition gesture is BOOT twice |
...
- **The unit in hand is a V2** (CO5300 panel + CST820 touch) — the replacement for the original V1, whose touch flex died to a cable yank in Aug 2026. Verified on hardware (kids_a18, Aug 2026): the CO5300 renders correctly on the SH8601 init list, so the `Panel_SH8601_AMOLED18` subclass is kept as-is; only the touch config changed (CST820 at 0x15, CST816-family so `TOUCH_NACKS_WHEN_IDLE = true` — the boot probe NACKs and that's normal). For a V1 unit: FT3168 at 0x38, `false`. If a future board stays black on this config, `lgfx::Panel_CO5300` (shipped by LovyanGFX) is the subclass to try before debugging init commands. On a live board the touch chip's I2C address tells the revisions apart.
...
- Same shared I2C bus for everything: SDA=15, SCL=14 (touch, QMI8658, AXP2101 PMU, PCF85063 RTC). Touch interrupt is GPIO21, unused — `touch.h` polls the touch chip's finger count instead (CST820 at 0x15 on this V2 unit).
...
`src/switcher_kids.cpp` (`kids_t2*`, `kids_a18*`) is a second roster built the same way — paint + zhi + the core + napoleon for small hands. One deliberate difference: **it polls only BOOT and never reads taps**, because every hosted sketch owns the glass (paint's tap cycles the brush color; zhi classifies tap-vs-scrub itself; the core's press is the jelly; napoleon's tap resets its layout), and a switcher-level `tapDetected()` racing paint's press/release timing would wipe a drawing on every crayon change. Its roster position persists under its own NVS key (`kidsketch`), so it and `switcher_t2` don't resume into each other's index when flashed over one another. Swapping away from paint discards the drawing (the incoming `setup()` repaints the shared framebuffer) — inherent to the roster model, not a bug.
