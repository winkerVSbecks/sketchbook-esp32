...
//
//   tap       play the original animation once — the stagger-bounce open,
//             the settle, then an eased close back to rest
//   hold      the stack squashes under the finger (squish starts once the
//             press outlives the tap window, so a tap doesn't flinch)
//   release   a damped spring rings the squash out — the inner cubes lag
//             the outer ones a few frames, which is what reads as jelly
//
...
// for each pixel inside the stack's bounding box, walk the six hexagons back
// to front, take analytic edge coverage (signed distance to the convex hull,
// the porting-patterns recipe generalised from the quarter-ellipse), lerp the
// gradient in RGB888, and blend. One to565Dither at the write keeps the
// gradients from banding in 565. No framebuffer readback anywhere.
// ============================================================================
...
static const uint32_t PLAY_MS  = 4000;   // the original loop duration
static const uint32_t CLOSE_MS = 400;    // eased return to rest after the hold

// The jelly. Squash approaches SQUISH_MAX with time constant SQUISH_TAU while
// held; release rings it out as q0 * e^(-t/RING_TAU) * cos(2pi*RING_HZ*t),
// each layer starting RING_LAG later than the one outside it.
static const float SQUISH_MAX = 0.30f;
static const float SQUISH_TAU = 0.12f;
static const float RING_HZ    = 2.5f;
static const float RING_TAU   = 0.50f;
...
// exactly this — written statelessly here so the eased close works too. The
// squash qk (positive = pressed flat, negative = stretched tall on the
// rebound) scales about the centre, bulging x by half of what it takes off y.
static void updateCuboids(float ph, const float *qk) {
  ...
  for (int k = 0; k < N_CUBOIDS; k++) {
    Cuboid &c = cuboids[k];
    const float p  = ph < DURATIONS[k] ? bounce(ph / DURATIONS[k]) : 1.0f;
    const float sx = 1.0f + 0.5f * qk[k];
    const float sy = 1.0f - qk[k];

    c.x0 = 1e9f; c.y0 = 1e9f; c.x1 = -1e9f; c.y1 = -1e9f;
    for (int i = 0; i < 6; i++) {
      c.vx[i] = cx + (c.sx[i] + DSIGN[i] * offX * p) * SCALE * sx;
      c.vy[i] = cy + (c.sy[i] - DSIGN[i] * offY * p) * SCALE * sy;
...
static inline uint32_t lerp888(uint32_t a, uint32_t b, float t) {
  ...
  const uint32_t v = (uint32_t)(ab + (bb - ab) * t);
  return (r << 16) | (g << 8) | v;
}
...
static void renderAll(float ph, const float *qk) {
  ...
  for (int py = by0; py < by1; py++) {
    ...
    for (int px = bx0; px < bx1; px++) {
      ...
      // Composite back to front, exactly the canvas draw order: largest
      // cuboid first, each blended over what's beneath by its coverage.
      uint32_t col = BG;
      bool hit = false;
      for (int k = 0; k < N_CUBOIDS; k++) {
        const Cuboid &c = cuboids[k];
        ...
        const uint32_t gc = lerp888(COLORS[k][0], COLORS[k][1], g);
        col = cover >= 1.0f ? gc : lerp888(col, gc, cover);
        hit = true;
      }
      if (hit) cv.drawPixel(px, py, to565Dither(col, px, py));
    }
  }
}
...
// original animation's playhead) and the squish (the jelly). File-scope so a
// switcher re-entry through setup() resets them — function-local statics
// would carry a half-finished wobble across sketches.
enum ShearMode  { SHEAR_IDLE, SHEAR_PLAY, SHEAR_HOLD, SHEAR_CLOSE };
enum SquishMode { SQ_QUIET, SQ_HELD, SQ_RING };

static ShearMode  shearMode = SHEAR_IDLE;
static SquishMode squishMode = SQ_QUIET;
static uint32_t   playStart = 0, closeStart = 0, ringStart = 0;
static uint32_t   pressAt = 0, lastStep = 0;
static float      phFrozen = 0.0f, phCloseFrom = 0.0f;
static float      q = 0.0f, q0 = 0.0f;
...
void setup() {
  ...
  static const float QZERO[N_CUBOIDS] = { 0 };
  const uint32_t t0 = wallMicros();
  renderAll(0.0f, QZERO);
...
void loop() {
  ...
  // whole stagger. setup() wrote frame 0, the loop covers 1/(N-1) .. 1.
  static const float QZERO[N_CUBOIDS] = { 0 };
  static int frame = 1;
  ...
  if (!down && wasDown) {
    if (now - pressAt < TAP_MS) {
      shearMode = SHEAR_PLAY;              // tap: run the pop from the top
      playStart = now;
    } else {
      q0         = q;                      // hold released: ring the squash out
      ringStart  = now;
      squishMode = fabsf(q0) > RING_DONE ? SQ_RING : SQ_QUIET;
      if (squishMode == SQ_QUIET) q = 0.0f;
      if (shearMode == SHEAR_HOLD) {       // and let the frozen shear close
        phCloseFrom = phFrozen;
        closeStart  = now;
        shearMode   = SHEAR_CLOSE;
      }
    }
  }
  wasDown = down;
  ...
  switch (shearMode) {
    case SHEAR_PLAY: {
      ...
      if (t >= 1.0f) { shearMode = SHEAR_CLOSE; phCloseFrom = 1.0f; closeStart = now; ph = 1.0f; }
      else           { ph = t; }
      break;
    }
    case SHEAR_HOLD: ph = phFrozen; break;
    case SHEAR_CLOSE: {
      const float t = (float)(now - closeStart) / (float)CLOSE_MS;
      if (t >= 1.0f) { shearMode = SHEAR_IDLE; ph = 0.0f; }
      else           { const float s = t * t * (3.0f - 2.0f * t); ph = phCloseFrom * (1.0f - s); }
      break;
    }
    default: ph = 0.0f; break;
  }
  ...
  if (squishMode == SQ_HELD) {
    ...
    q += (SQUISH_MAX - q) * (1.0f - expf(-dt / SQUISH_TAU));
    for (int k = 0; k < N_CUBOIDS; k++) qk[k] = q;
  } else if (squishMode == SQ_RING) {
    const float t = (float)(now - ringStart) * 0.001f;
    for (int k = 0; k < N_CUBOIDS; k++) {
      const float tk = t - (float)k * RING_LAG;
      qk[k] = tk <= 0.0f ? q0
                         : q0 * expf(-tk / RING_TAU) * cosf(6.2831853f * RING_HZ * tk);
    }
    q = qk[0];   // so a re-press mid-wobble squashes from where the jelly is
    const float tInner = t - (float)(N_CUBOIDS - 1) * RING_LAG;
    if (tInner > 0.0f && fabsf(q0) * expf(-tInner / RING_TAU) < RING_DONE) {
      squishMode = SQ_QUIET;
      q = 0.0f;
      for (int k = 0; k < N_CUBOIDS; k++) qk[k] = 0.0f;
    }
  } else {
    for (int k = 0; k < N_CUBOIDS; k++) qk[k] = 0.0f;
  }
  ...
  if (active || !restDrawn) {
    ...
    if (millis() - reportAt >= 1000) {
      reportAt = millis();
      Serial.printf("ph %.3f  q %.3f  draw %luus\n", ph, qk[0], (unsigned long)drawUs);
    }
  } else {
    delay(8);
  }
